...
class _IslamicOnboardingState extends State<IslamicOnboarding> {
  ...
  /// Enhanced location permission request with iOS-specific handling
  ...
  //     // Check if location services are enabled (iOS specific check)
  //     if (Platform.isIOS) {
  //       PermissionStatus serviceStatus = PermissionStatus.denied; 
  //       if (serviceStatus.isDenied) {
  ...
  //       await _getLocationAndSetupPrayerTimes();
  //     }
...
void _requestLocationPermission() async {
  ...
  try {
    ...
    // Optionally save the status to SharedPreferences
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('location_permission_granted', permissionStatus.isGranted);

    // If granted, try to fetch location and prayer times
    if (permissionStatus.isGranted) {
      await _getLocationAndSetupPrayerTimes();
    } else {
      // Skip fetching location, just continue
      _completeOnboarding();
    }
  } catch (e) {
  ...
  /// Get location and setup prayer times
  Future<void> _getLocationAndSetupPrayerTimes() async {
    try {
      // Use your existing LocationService
      ...
      String cityName = locationInfo['city'] ?? 'Unknown City';
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Location set successfully! Prayer times for $cityName are now available.',
            ),
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
            'Location access has been permanently denied. Please enable location permission in your device settings to get accurate prayer times.',
          ),
          actions: [
            TextButton(
              onPressed: () {
  ...
  void _showPermissionDeniedMessage() {
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Location permission denied. You can enable it later in settings to get accurate prayer times.',
          ),
    ...
    // Auto-complete onboarding after showing message
  ...
  void _showLocationErrorMessage() {
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Failed to get location. You can try again later in settings.',
          ),
          backgroundColor: AppTheme.lightTheme.colorScheme.error,
          action: SnackBarAction(
            label: 'Continue',
